...
static bool fllama_verbose = false;
...
struct fllama_context
{
    ...
    bool loadModel(common_params &params_)
    {
        ...
        std::tie(model, ctx) = std::tie(init_result.model, init_result.context);
        if (model == nullptr)
        {
    ...
    completion_token_output nextToken()
    {
        ...
        {
            // stopping_word = common_token_to_piece(ctx, embd.back());
            has_next_token = false;
            stopped_eos = true;
