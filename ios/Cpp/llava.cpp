...
static bool clip_llava_handle_patches(clip_ctx * ctx_clip, std::vector<float *> & image_embd_v, struct clip_image_grid_shape grid_shape, float * image_embd_out, int * n_img_pos_out) {
...

    {
        ...
        ctx_size += 1024*1024 * lm_ggml_type_size(LM_GGML_TYPE_F32);
    }
    ...
    */
    ...
    model.ctx = lm_ggml_init(params);

    struct lm_ggml_tensor * image_features = lm_ggml_new_tensor_3d(model.ctx, LM_GGML_TYPE_F32, clip_n_mmproj_embd(ctx_clip), clip_n_patches(ctx_clip), num_images - 1); // example: 4096 x 576 x 4
    // lm_ggml_tensor_printf(image_features,"image_features",__LINE__,false,false);
    ...
    struct lm_ggml_cgraph  * gf = lm_ggml_new_graph(model.ctx);
    size_t size_ele = lm_ggml_type_size(LM_GGML_TYPE_F32);

    struct lm_ggml_tensor *image_features_patchview = lm_ggml_view_4d(model.ctx, image_features,
    ...
    // lm_ggml_tensor_printf(image_features_patchview,"image_features_patchview",__LINE__,false,false);
    struct lm_ggml_tensor *permuted_cont = lm_ggml_cont(model.ctx, lm_ggml_permute(model.ctx, image_features_patchview, 0, 2, 1, 3));
    /**
    ...
    // lm_ggml_tensor_printf(permuted_cont,"permuted_cont",__LINE__,false,false);
    struct lm_ggml_tensor *flatten = lm_ggml_view_2d(model.ctx, permuted_cont, clip_n_mmproj_embd(ctx_clip), num_patches_height * num_patches_width * num_patches_per_side * num_patches_per_side,  size_ele * clip_n_mmproj_embd(ctx_clip), 0);
    // lm_ggml_tensor_printf(flatten,"flatten",__LINE__,false,false);
    lm_ggml_build_forward_expand(gf, flatten);
    lm_ggml_graph_compute_with_ctx(model.ctx, gf, 1);
    struct lm_ggml_tensor* result = lm_ggml_graph_node(gf, -1);
...
static bool encode_image_with_clip(clip_ctx * ctx_clip, int n_threads, const clip_image_u8 * img, float * image_embd, int * n_img_pos) {
    ...
    const int64_t t_img_enc_start_us = lm_ggml_time_us();
    ...
    if (clip_is_minicpmv(ctx_clip)) {
        ...
        for (size_t i = 0; i < img_res_v.size; i++) {
            const int64_t t_img_enc_step_start_us = lm_ggml_time_us();
            ...
            const int64_t t_img_enc_steop_batch_us = lm_ggml_time_us();
            LOG_INF("%s: step %d of %d encoded in %8.2f ms\n", __func__, (int)i+1, (int)img_res_v.size, (t_img_enc_steop_batch_us - t_img_enc_step_start_us) / 1000.0);
        }
        const int64_t t_img_enc_batch_us = lm_ggml_time_us();
    ...
    const int64_t t_img_enc_end_us = lm_ggml_time_us();
